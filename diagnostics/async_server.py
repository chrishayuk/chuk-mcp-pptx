...
from chuk_mcp_pptx.server import (
    ...
    pptx_add_chart, pptx_add_pie_chart, pptx_save, pptx_get_info,
    pptx_list, pptx_switch, pptx_delete
)
import json
...
async def test_async_server():
    ...
    result = await pptx_add_title_slide(
        title="Async Test Presentation",
        subtitle="Testing async/await functionality"
    )
    print(f"   ✅ {result}")
