...
class TestCreateQuickDeck:
    ...
    @pytest.mark.asyncio
    async def test_create_quick_deck_creates_presentation(self, semantic_tools, mock_presentation_manager):
        """Test that quick deck creates presentation."""
        await semantic_tools['pptx_create_quick_deck'](
            name='new_deck',
            title='New Deck'
        )
        # Manager's create method should have been called
        mock_presentation_manager.create.assert_called()
...
class TestAddMetricsDashboard:
    ...
    @pytest.mark.asyncio
    async def test_add_metrics_dashboard_grid_layout(self, semantic_tools, mock_presentation_manager):
    ...
    @pytest.mark.asyncio
    async def test_add_metrics_dashboard_row_layout(self, semantic_tools, mock_presentation_manager):
    ...
    @pytest.mark.asyncio
    async def test_add_metrics_dashboard_with_changes(self, semantic_tools, mock_presentation_manager):
    ...
    @pytest.mark.asyncio
    async def test_add_metrics_dashboard_with_theme(self, semantic_tools, mock_presentation_manager):
...
class TestAddContentGrid:
    ...
    @pytest.mark.asyncio
    async def test_add_content_grid_column_clamping(self, semantic_tools, mock_presentation_manager):
...
class TestAddComparisonSlide:
    ...
    async def test_add_comparison_slide_basic(self, semantic_tools, mock_presentation_manager):
        ...
        assert isinstance(result, str)
        assert 'Build In-House' in result or 'Buy Solution' in result or 'comparison' in result.lower()

    @pytest.mark.asyncio
    async def test_add_comparison_slide_empty_items(self, semantic_tools, mock_presentation_manager):
...
class TestIntegration:
    ...
    @pytest.mark.asyncio
    async def test_workflow_create_deck_with_slides(self, semantic_tools, mock_presentation_manager):
        """Test complete workflow: create deck → add metrics → add comparison."""
        # Create quick deck
        create_result = await semantic_tools['pptx_create_quick_deck'](
            name='workflow_test',
            title='Test Deck',
            subtitle='Testing workflow'
        )
