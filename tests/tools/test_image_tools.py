    ...
    async def test_add_background_image(self, image_tools, mock_presentation_manager):
        ...
        with patch('pathlib.Path.exists', return_value=True):
            result = await image_tools['pptx_add_background_image'](
                slide_index=0,
                image_path="/fake/background.jpg"
            )
            assert isinstance(result, str)
...
class TestAddImageWithCaption:
    ...
    @pytest.mark.asyncio
    async def test_add_image_with_caption_custom_position(self, image_tools, mock_presentation_manager):
    ...
    @pytest.mark.asyncio
    async def test_add_image_with_caption_invalid_slide(self, image_tools, mock_presentation_manager):
        """Test error with invalid slide index."""
        result = await image_tools['pptx_add_image_with_caption'](
            slide_index=999,
            image_path="/fake/img.png",
    ...
    async def test_replace_image_basic(self, image_tools, mock_presentation_manager):
        ...
        with patch('pathlib.Path.exists', return_value=True):
            result = await image_tools['pptx_replace_image'](
                slide_index=0,
                old_image_index=0,
                new_image_path="/fake/new_image.png"
            )
            assert isinstance(result, str)
...
class TestAddImagePlaceholder:
    ...
    @pytest.mark.asyncio
    async def test_add_image_placeholder_custom_colors(self, image_tools, mock_presentation_manager):
    ...
    @pytest.mark.asyncio
    async def test_add_image_placeholder_invalid_slide(self, image_tools, mock_presentation_manager):
        """Test error with invalid slide index."""
        result = await image_tools['pptx_add_image_placeholder'](
            slide_index=999
        )
...
class TestHelperFunctions:
    ...
    def test_add_image_base64(self, mock_presentation_manager):
        """Test add_image helper with base64."""
        slide = mock_presentation_manager.get_current().slides[0]
        base64_data = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="

        result = add_image(slide, base64_data, 1.0, 1.0)
        assert result is not None

    def test_add_image_file_path(self, mock_presentation_manager):
    ...
    def test_add_image_with_width_only(self, mock_presentation_manager):
    ...
    def test_add_image_with_height_only(self, mock_presentation_manager):
        """Test add_image with height only."""
        slide = mock_presentation_manager.get_current().slides[0]
    ...
    def test_add_text_box_with_style_caption(self, mock_presentation_manager):
        """Test text box with caption style."""
        slide = mock_presentation_manager.get_current().slides[0]

        result = add_text_box_with_style(
            slide, 1.0, 5.0, 4.0, 0.5,
            "Test caption",
            style_preset="caption"
        )
        assert result is not None

    def test_add_text_box_with_style_title(self, mock_presentation_manager):
        """Test text box with title style."""
        slide = mock_presentation_manager.get_current().slides[0]

        result = add_text_box_with_style(
            slide, 1.0, 1.0, 6.0, 1.0,
            "Test Title",
            style_preset="title"
        )
        assert result is not None

    def test_add_text_box_with_style_body(self, mock_presentation_manager):
        """Test text box with body style."""
        slide = mock_presentation_manager.get_current().slides[0]

        result = add_text_box_with_style(
            slide, 1.0, 2.0, 5.0, 2.0,
            "Body text content",
            style_preset="body"
        )
        assert result is not None
