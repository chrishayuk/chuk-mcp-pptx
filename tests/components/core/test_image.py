...
class TestImageComponent:
    ...
    def test_render_basic(self, slide, test_image_path):
        """Test basic rendering."""
        image = Image(image_source=test_image_path)
        result = image.render(slide, left=1.0, top=1.0, width=4.0)
        assert result is not None
        assert len(slide.shapes) == 1

    def test_render_with_width_only(self, slide, test_image_path):
        """Test rendering with width only (maintains aspect ratio)."""
        image = Image(image_source=test_image_path)
        result = image.render(slide, left=1.0, top=1.0, width=4.0)
        assert result is not None

    def test_render_with_height_only(self, slide, test_image_path):
        """Test rendering with height only (maintains aspect ratio)."""
        image = Image(image_source=test_image_path)
        result = image.render(slide, left=1.0, top=1.0, height=3.0)
        assert result is not None

    def test_render_with_width_and_height(self, slide, test_image_path):
        """Test rendering with both width and height."""
        image = Image(image_source=test_image_path)
        result = image.render(slide, left=1.0, top=1.0, width=4.0, height=3.0)
        assert result is not None

    def test_render_no_dimensions(self, slide, test_image_path):
        """Test rendering without dimensions (uses original size)."""
        image = Image(image_source=test_image_path)
        result = image.render(slide, left=1.0, top=1.0)
        assert result is not None

    def test_render_with_shadow(self, slide, test_image_path):
    ...
    def test_render_base64_image(self, slide, base64_image):
        """Test rendering with base64 image data."""
        image = Image(image_source=base64_image)
        result = image.render(slide, left=1.0, top=1.0, width=3.0)
        assert result is not None

    def test_render_base64_no_processing(self, slide, base64_image):
    ...
    def test_file_not_found(self, slide):
        """Test error handling for non-existent file."""
        image = Image(image_source="/path/to/nonexistent/image.png")
        with pytest.raises(FileNotFoundError):
            image.render(slide, left=1.0, top=1.0)
    ...
    def test_apply_filters_with_rgba_image(self, slide, test_rgba_image_path):
    ...
    def test_apply_sepia(self, test_image_path):
    ...
    def test_apply_filters_all_combinations(self, test_image_path):
    ...
    def test_needs_processing_detection(self, test_image_path):
        ...
        assert not (
            image1.blur_radius > 0 or
            image1.grayscale or
            image1.sepia or
            image1.brightness != 1.0 or
            image1.contrast != 1.0 or
            image1.saturation != 1.0 or
            image1.sharpen or
            image1.invert
        )
...
class TestImageEffects:
    ...
    def test_shadow_properties(self, slide, test_image_path):
    ...
    def test_multiple_images_on_slide(self, slide, test_image_path):
        ...
        result1 = image1.render(slide, left=1.0, top=1.0, width=2.0)
        result2 = image2.render(slide, left=3.5, top=1.0, width=2.0)
        result3 = image3.render(slide, left=6.0, top=1.0, width=2.0)
...
class TestImageEdgeCases:
    ...
    def test_extreme_blur(self, slide, test_image_path):
        """Test with very high blur radius."""
        image = Image(image_source=test_image_path, blur_radius=50)
        result = image.render(slide, left=1.0, top=1.0, width=4.0)
        assert result is not None

    def test_extreme_brightness(self, slide, test_image_path):
        ...
        result2 = image2.render(slide, left=4.0, top=1.0, width=2.0)
        assert result2 is not None

    def test_extreme_contrast(self, slide, test_image_path):
        """Test with extreme contrast values."""
        image = Image(image_source=test_image_path, contrast=5.0)
        result = image.render(slide, left=1.0, top=1.0, width=4.0)
        assert result is not None

    def test_zero_saturation(self, slide, test_image_path):
        """Test with zero saturation (should be like grayscale)."""
        image = Image(image_source=test_image_path, saturation=0.0)
        result = image.render(slide, left=1.0, top=1.0, width=4.0)
        assert result is not None

    def test_grayscale_then_sepia(self, slide, test_image_path):
    ...
    def test_all_filters_combined(self, slide, test_image_path):
