...
from chuk_mcp_pptx.components.charts.scatter_bubble import ScatterChart, BubbleChart, Matrix3DChart
...
class TestScatterChartRendering:
    ...
    def test_render_smooth_markers_variant(self, slide):
        ...
        chart = ScatterChart(
            ...
            variant="smooth_markers"
        )
        result = chart.render(slide)
        # Smooth markers variant
        assert result.chart_type in [XL_CHART_TYPE.XY_SCATTER_SMOOTH_NO_MARKERS, XL_CHART_TYPE.XY_SCATTER_SMOOTH]
    ...
    def test_render_lines_markers_variant(self, slide):
        ...
        chart = ScatterChart(
            ...
            variant="lines_markers"
        )
        result = chart.render(slide)
        # Lines markers variant
        assert result.chart_type in [XL_CHART_TYPE.XY_SCATTER_LINES_NO_MARKERS, XL_CHART_TYPE.XY_SCATTER_LINES]
    ...
    def test_render_with_trendline(self, slide):
        ...
        chart = ScatterChart(
            series_data=[
                {"x_values": [1, 2, 3, 4, 5], "y_values": [2, 4, 3, 5, 6]}
            ],
            show_trendline=True
        )
        result = chart.render(slide)
...
class TestBubbleChartRendering:
    ...
    def test_render_basic_bubble(self, slide):
        ...
        chart = BubbleChart(
            series_data=[
                {"name": "Series 1", "points": [[1, 2, 10], [2, 3, 20], [3, 4, 30]]}
            ],
            title="Bubble Chart"
        )
        result = chart.render(slide)
    ...
    def test_render_custom_position(self, slide):
        ...
        chart = BubbleChart(
            series_data=[
                {"points": [[1, 1, 10], [2, 2, 20]]}
            ]
        )
        result = chart.render(slide, left=1.5, top=2.5)
        assert len(slide.shapes) > 0
...
class TestScatterBubbleEdgeCases:
    ...
    def test_scatter_decimal_values(self, slide):
        ...
        chart = ScatterChart(
            series_data=[
                {"x_values": [1.5, 2.7, 3.2], "y_values": [1.1, 2.3, 3.9]}
            ]
        )
        result = chart.render(slide)
        assert result is not None
