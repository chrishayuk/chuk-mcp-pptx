...
from chuk_mcp_pptx.components.core import (
    Shape, Connector, Image, ProcessFlow, CycleDiagram, HierarchyDiagram
)
...
class TestShape:
    ...
    def test_shape_types(self, slide, dark_theme):
        ...
        shape_types = [
            ...
            "diamond", "triangle", "star", "hexagon", "heart"
        ]
...
class TestConnector:
    ...
    def test_connector_with_arrows(self, slide, dark_theme):
        ...
        connector1 = Connector(
            start_x=1.0, start_y=2.0, end_x=5.0, end_y=2.0,
            arrow_end=True,
            arrow_start=False,
            theme=dark_theme
        )
        ...
        connector2 = Connector(
            start_x=1.0, start_y=3.0, end_x=5.0, end_y=3.0,
            arrow_end=False,
            arrow_start=True,
            theme=dark_theme
        )
        ...
        connector3 = Connector(
            start_x=1.0, start_y=4.0, end_x=5.0, end_y=4.0,
            arrow_end=True,
            arrow_start=True,
            theme=dark_theme
        )
    ...
    def test_connector_with_colors(self, slide, dark_theme):
        ...
        # Hex color
        connector1 = Connector(
            start_x=1.0, start_y=2.0, end_x=5.0, end_y=3.0,
            line_color="#FF0000",
            theme=dark_theme
        )
        rendered1 = connector1.render(slide)
        ...
        # Semantic color
        connector2 = Connector(
            start_x=1.0, start_y=3.0, end_x=5.0, end_y=4.0,
...
class TestImage:
    ...
    def test_init(self, temp_image):
        ...
        assert image.shadow == False

    def test_render_from_file(self, slide, temp_image, dark_theme):
        """Test rendering image from file path."""
        image = Image(image_source=temp_image, theme=dark_theme)
        rendered = image.render(slide, left=1, top=1, width=3)
        assert rendered is not None

    def test_render_with_width_and_height(self, slide, temp_image, dark_theme):
        """Test rendering with both width and height."""
        image = Image(image_source=temp_image, theme=dark_theme)
        rendered = image.render(slide, left=1, top=1, width=4, height=3)
        assert rendered is not None

    def test_render_with_width_only(self, slide, temp_image, dark_theme):
        """Test rendering with width only (maintains ratio)."""
        image = Image(image_source=temp_image, theme=dark_theme)
        rendered = image.render(slide, left=1, top=1, width=4)
        assert rendered is not None

    def test_render_with_height_only(self, slide, temp_image, dark_theme):
        """Test rendering with height only (maintains ratio)."""
        image = Image(image_source=temp_image, theme=dark_theme)
        rendered = image.render(slide, left=1, top=1, height=3)
        assert rendered is not None

    def test_render_with_shadow(self, slide, temp_image, dark_theme):
        """Test rendering with shadow effect."""
        image = Image(image_source=temp_image, shadow=True, theme=dark_theme)
        rendered = image.render(slide, left=1, top=1, width=3)
        assert rendered is not None

    def test_file_not_found(self, slide, dark_theme):
        """Test error handling for non-existent file."""
        image = Image(image_source="/nonexistent/path.png", theme=dark_theme)
        with pytest.raises(FileNotFoundError):
            image.render(slide, left=1, top=1)

    def test_blur_filter(self, slide, temp_image, dark_theme):
        """Test blur filter."""
        image = Image(image_source=temp_image, blur_radius=10, theme=dark_theme)
        rendered = image.render(slide, left=1, top=1, width=3)
        assert rendered is not None

    def test_grayscale_filter(self, slide, temp_image, dark_theme):
        """Test grayscale filter."""
        image = Image(image_source=temp_image, grayscale=True, theme=dark_theme)
        rendered = image.render(slide, left=1, top=1, width=3)
        assert rendered is not None

    def test_sepia_filter(self, slide, temp_image, dark_theme):
        """Test sepia filter."""
        image = Image(image_source=temp_image, sepia=True, theme=dark_theme)
        rendered = image.render(slide, left=1, top=1, width=3)
        assert rendered is not None

    def test_brightness_filter(self, slide, temp_image, dark_theme):
        """Test brightness adjustment."""
        image = Image(image_source=temp_image, brightness=1.5, theme=dark_theme)
        rendered = image.render(slide, left=1, top=1, width=3)
        assert rendered is not None

    def test_contrast_filter(self, slide, temp_image, dark_theme):
        """Test contrast adjustment."""
        image = Image(image_source=temp_image, contrast=1.8, theme=dark_theme)
        rendered = image.render(slide, left=1, top=1, width=3)
        assert rendered is not None

    def test_saturation_filter(self, slide, temp_image, dark_theme):
        """Test saturation adjustment."""
        image = Image(image_source=temp_image, saturation=2.0, theme=dark_theme)
        rendered = image.render(slide, left=1, top=1, width=3)
        assert rendered is not None

    def test_sharpen_filter(self, slide, temp_image, dark_theme):
        """Test sharpen filter."""
        image = Image(image_source=temp_image, sharpen=True, theme=dark_theme)
        rendered = image.render(slide, left=1, top=1, width=3)
        assert rendered is not None

    def test_invert_filter(self, slide, temp_image, dark_theme):
        """Test invert filter."""
        image = Image(image_source=temp_image, invert=True, theme=dark_theme)
        rendered = image.render(slide, left=1, top=1, width=3)
        assert rendered is not None

    def test_combined_filters(self, slide, temp_image, dark_theme):
...
class TestProcessFlow:
    ...
    def test_render(self, slide, dark_theme):
        ...
        process = ProcessFlow(
            items=["Research", "Design", "Develop", "Test"],
            theme=dark_theme
        )
