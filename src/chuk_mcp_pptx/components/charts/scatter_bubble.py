    ...
    def validate_data(self) -> Tuple[bool, Optional[str]]:
        ...
        for i, series in enumerate(self.series_data):
            ...
            if len(x_values) != len(y_values):
                return False, f"Series {i}: x_values ({len(x_values)}) and y_values ({len(y_values)}) must have same length"
    ...
    def validate_data(self) -> Tuple[bool, Optional[str]]:
        ...
        for i, series in enumerate(self.series_data):
            ...
            for j, point in enumerate(points):
                ...
                x, y, size = point
                if not isinstance(x, (int, float)) or not isinstance(y, (int, float)) or not isinstance(size, (int, float)):
